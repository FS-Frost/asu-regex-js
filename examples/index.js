...
function parseTags(text, tags) {
  const resultT = text.match(regexTagT);
  if (resultT && resultT[0]) {
    const tagNameSource2 = text.substring(1);
    if (tagNameSource2.startsWith(TagName.t)) {
      const r = createRegExp(reTGeneral);
      const a = resultT[0].match(r)?.groups;
    ...
    text = text.substring(resultT[0].length);
    if (text.length > 0) {
      parseTags(text, tags);
    }
    return tags;
  }
  const result = text.match(regexTags);
  if (!result || !result[0]) {
    return tags;
  }
